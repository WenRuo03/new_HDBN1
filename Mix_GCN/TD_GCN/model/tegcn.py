...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=6,
        ...
        #3D-TA all
        if 1:
            self.l1 = TCNC_GCN_unit(6, 64, A, residual=False, adaptive=adaptive, attention=attention)
...
if __name__ == '__main__':
    inp = torch.randn((32, 6, 300, 17))
    A = torch.randn((3, 17, 17))
    net = TCNC_GCN_unit(6, 64, A)
    out = net(inp)
    print(out.shape)
