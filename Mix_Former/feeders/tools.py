...
def valid_crop_resize(data_numpy, valid_frame_num, p_interval, window):
    ...
    # 检查裁剪后的长度是否有效
    if cropped_length <= 0 or data.shape[1] == 0:
        # print(f"Invalid cropped length: {cropped_length}, skipping this data.")
        return None  # 跳过无效数据
    ...
    # 检查插值是否会导致无效尺寸
    if cropped_length == 0:
        # print("Cropped length is 0, skipping this data.")
        return None  # 跳过无效数据
    ...
    if data.size(1) == 0:
        # print("Invalid interpolation result, skipping this data.")
        return None  # 跳过无效数据
